 ...
 *              Monta filesystems virtuales, define entorno real (PATH),
 *              y gestiona la consola principal con respawn.
 * ==============================================================================
 */
...
void mount_virtual_filesystems() {
    ...
    if (mount("devpts", "/dev/pts", "devpts", 0, NULL) != 0) {
        perror("[!] Error montando /dev/pts (no crítico)");
    }

    mkdir("/run", 0755);
    if (mount("tmpfs", "/run", "tmpfs", 0, NULL) != 0) {
        perror("[!] Error montando /run (no crítico)");
    }
}
